...
// The below library has been modified to allow asynchronous query of neighbours and multiple destinations
...
abstract class Graph<T extends Node<T>> {
  ...
  ///
  /// Optimize this first.
  num getHeuristicDistance(T a, Set<T> goals);

  /// Given a node, return all connected nodes.
  Future<Iterable<T>> getNeighboursOf(T node);
}
...
class AStar<T extends Node<T>> {
  ...
  /// TODO: Optional weighing for suboptimal, but faster path finding.
  /// http://en.wikipedia.org/wiki/A*_search_algorithm#Bounded_relaxation
  Future<Queue<T>> findPath(T start, Set<T> goals) async {
    if (!_zeroed) {
      _zeroNodes();
    }
    ...
    while (_open.isNotEmpty) {
      ...
      if (goals.contains(currentNode)) {
        ...
        final path = Queue<T>()..add(currentNode);
      ...
      for (final candidate in await graph.getNeighboursOf(currentNode)) {
        ...
        if (distance != double.infinity) {
          ...
          if (!candidate._isInOpenSet) {
            ...
            final h = graph.getHeuristicDistance(candidate, goals);
